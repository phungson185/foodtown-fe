...
const ProductForm = ({ updatingProduct, cancellingAddingProduct }) => {
  ...
  return (
    <div className="product__form-container">
      ...
      <img src={productImage} alt="" className="fixed-image" />
      <div className="product__form-buttons">
        <Button variant="outlined" onClick={onCancelAddingProduct}>
          CANCEL
        </Button>
        <Button variant="contained" onClick={updatingProduct.name ? onUpdateProduct : onAddProduct}>
          {updatingProduct.name ? 'UPDATE' : 'ADD'}
        </Button>
      </div>
    </div>
  )
