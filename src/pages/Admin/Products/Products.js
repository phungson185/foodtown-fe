...
const Products = () => {
  ...
  const [filterField, setFilterField] = useState(LIKE)
        ...
        <>
          <div className='products__management-header'>
            <div className='products__management-filters'>
              <TextField label="Search Product..." variant="outlined" size="small" value={searchingName} onChange={(e) => setSearchingName(e.target.value)} fullWidth />
              <Box sx={{ width: 140 }}>
            ...
            <Button variant="contained" onClick={onAddingProduct}>Add Product</Button>
          </div>
          {
            products ?
            products?.map(product => <Product product={product} updateProduct={setIsViewingProductList} setUpdatingProduct={setUpdatingProduct}/>)
            :
            <></>
          }
          <div className='products__management-pagination'>
            <Pagination count={10} page={page} onChange={(event, value) => setPage(value)} showFirstButton showLastButton />
          </div>
        </>
        :
        <ProductForm cancellingAddingProduct={setIsViewingProductList} updatingProduct={updatingProduct} />
      }
    </div>
  )
}
